...
public class RLContext : DbContext
{
    ...
    protected override async void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            typeBuilder.HasKey(u => u.UserId);
            typeBuilder.HasMany(u => u.PlanProcedureUsers).WithOne(ppu => ppu.User).HasForeignKey(ppu => ppu.UserId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        builder.Entity<PlanProcedure>(typeBuilder =>
        {
            ...
            typeBuilder.HasMany(pp => pp.PlanProcedureUsers).WithOne(ppu => ppu.PlanProcedure).HasForeignKey(ppu => new { ppu.PlanId, ppu.ProcedureId }).OnDelete(DeleteBehavior.Cascade);
        });

        builder.Entity<PlanProcedureUser>(typeBuilder =>
        {
            typeBuilder.HasKey(ppu => new { ppu.PlanId, ppu.ProcedureId, ppu.UserId});
            typeBuilder.HasOne(ppu => ppu.PlanProcedure).WithMany(pp => pp.PlanProcedureUsers).HasForeignKey(ppu => new { ppu.PlanId, ppu.ProcedureId }).OnDelete(DeleteBehavior.Cascade);
            typeBuilder.HasOne(ppu => ppu.User).WithMany(u => u.PlanProcedureUsers).HasForeignKey(ppu => ppu.UserId).OnDelete(DeleteBehavior.Cascade);
        });
